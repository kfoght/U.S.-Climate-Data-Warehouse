...
with_llm_org as (
  ...
  from base_ccf
  left join `kiaraerica`.`dbt_us_climate_int`.`tmp_ccf_facilities_llm_org` as llm
...
with_standardized_org as (
  ...
  from with_llm_org
  inner join `kiaraerica`.`dbt_us_climate_int`.`tmp_ccf_organization_name_mapping` as mapping
...
filtered as (
  ...
  inner join `kiaraerica`.`dbt_us_climate_int`.`Organizations` o
