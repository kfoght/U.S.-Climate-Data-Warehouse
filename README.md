...
## Final Project
A fuzzy matching, embedding-based approach, was implemented to normalize organization names within the Facility GHG emissions dataset. Previously, an LLM was used to directly normalize organization names by inferring the most canonical form from raw inputs. The pipeline began by generating text embeddings for each organization name using BigQuery’s `ml.generate_embedding()` function. A cosine distance threshold (≤ 0.075) was applied during nearest neighbor search to identify similar name pairs, which were then grouped into clusters—each assumed to represent a unique entity. To determine which name to retain from each cluster, an LLM was prompted to evaluate the grouped names and return the most representative one.

- **fin-us-climate-fuzzy-matching-cleaned.ipynb**: Renderable version of the fuzzy matching notebook for viewing on GitHub.
- **fin-us-climate-fuzzy-matching.ipynb**: Implements fuzzy matching, embedding-based, clustering and normalization of Facility GHG organization names using vector similarity search in BigQuery.
